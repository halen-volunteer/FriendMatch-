...
import { Check, Close, Loading, RefreshLeft, View, Warning } from '@element-plus/icons-vue'
...
const emit = defineEmits(['refresh', 'process', 'view-detail', 'update:currentPage', 'update:pageSize'])
          ...
          <el-table-column label="操作" width="300" fixed="right">
            <template #default="{ row }">
              <div class="report-table__actions">
                <el-button type="primary" plain size="small" @click="emit('view-detail', row.id)">
                  <el-icon><View /></el-icon>
                  <span>查看详情</span>
                </el-button>
                <template v-if="row.reportStatus === 0">
                  <el-button type="success" size="small" @click="emit('process', row.id, 1)">
                    <el-icon><Check /></el-icon>
                    <span>确认违规</span>
                  </el-button>
                  <el-button type="danger" size="small" @click="emit('process', row.id, 2)">
