...
<template>
  <div class="team-card" @click="emit('view', team)">
    ...
    <p v-if="team.teamIntro" class="team-intro">{{ team.teamIntro }}</p>
    <TagList v-if="team.teamTags" :tags="team.teamTags.split(',').filter(Boolean)" />
...
<style scoped>
.team-card { border-radius: 20px; padding: 18px; }
...
.team-intro { font-size: 12px; color: var(--wx-text-2); margin: 8px 0; line-height: 1.6; }
