...
const recommendTabs = [
  ...
  { key: 'teams', label: '发现队伍', path: '/recommend', query: { section: 'teams' } },
]

const noticeTabs = [
  { key: 'all', label: '全部通知', path: '/notices', query: { section: 'all' } },
  { key: 'friend', label: '好友通知', path: '/notices', query: { section: 'friend' } },
  { key: 'team', label: '队伍通知', path: '/notices', query: { section: 'team' } },
...
const otherTabs = [
  ...
  { key: 'appeals', label: '申诉中心', path: '/appeals' },
]
    ...
    <aside v-if="isListContextRoute" class="desktop-middle">
      ...
      <div v-else class="middle-panel">
        <header class="middle-panel__header">
          <h3>{{ middleTitle }}</h3>
          <p v-if="isFriendsRoute">左侧选择联系人、申请或队伍，右侧展示对应详情。</p>
          <p v-else-if="isRecommendRoute">根据入口切换用户推荐和队伍推荐。</p>
          <p v-else-if="isNoticesRoute">按通知类型筛选消息，提升处理效率。</p>
          <p v-else-if="isOtherRoute">设备、账号、反馈和申诉能力统一收口在这里。</p>
        </header>
