...
function formatPunishEndTime(row) {
  ...
  return formatDateTime(row.punishEndTime, fallbackText)
}
...
async function submitPunish() {
  const payload = {
    ...
    msgId: punishForm.value.msgId ? Number(punishForm.value.msgId) : undefined,
      ...
      <template v-else>
        ...
        <el-table v-else :data="punishRecords" stripe class="full-width-table">
          ...
          <el-table-column label="开始时间" width="170">
            <template #default="{ row }">
              {{ formatDateTime(row.punishStartTime) }}
            </template>
          </el-table-column>
