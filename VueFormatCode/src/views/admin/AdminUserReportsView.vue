...
import { getAdminUserReportContext, getAdminUserReports, handleAdminUserReport } from '@/api/admin'
...
const {
  ...
  confirmViolationNote: '用户举报成立，已按流程执行处罚。',
  confirmNoViolationNote: '用户举报不成立，已驳回该举报。',
})
...
async function openDetail(reportId) {
  ...
  const response = await getAdminUserReportContext(reportId).catch(() => null)
      ...
      <div v-else-if="currentDetail" class="stack-list">
        <el-card shadow="never">
          <div class="detail-grid">
            ...
            <div><strong>被举报用户 ID：</strong>{{ getSafeValue(currentDetail.report?.reportedUserId) }}</div>
        ...
        <el-card shadow="never">
          <template #header>被举报用户资料</template>
          <div class="detail-grid">
            <div><strong>用户 ID：</strong>{{ getSafeValue(currentDetail.reportedUser?.id) }}</div>
            <div><strong>账号：</strong>{{ getSafeValue(currentDetail.reportedUser?.userAccount) }}</div>
            <div><strong>昵称：</strong>{{ getSafeValue(currentDetail.reportedUser?.userNickname) }}</div>
            <div><strong>处罚状态：</strong>{{ getSafeValue(currentDetail.reportedUser?.globalPunishType, 0) }}</div>
          </div>
          <div class="detail-block">
            <strong>个人简介：</strong>
            <div class="admin-table-text">{{ getTextValue(currentDetail.reportedUser?.userIntro) }}</div>
          </div>
          <div class="detail-block">
            <strong>用户标签：</strong>
            <div class="admin-table-text">{{ getTextValue(currentDetail.reportedUser?.userTags) }}</div>
          </div>
        </el-card>

        <el-card shadow="never">
          <template #header>
            举报前 {{ chatContext.windowMinutes || 5 }} 分钟私聊上下文
            <span v-if="chatContext.truncated" class="detail-tip">仅展示前 {{ contextMessages.length }} 条，完整命中 {{ chatContext.totalCount }} 条</span>
          </template>
          <el-empty v-if="!contextMessages.length" description="该时间窗口内暂无私聊记录" />
