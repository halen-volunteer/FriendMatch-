...
const isSelf = () => String(authStore.userId) === String(route.params.userId)
async function load() { loading.value = true; try { const res = await getUserProfile(route.params.userId); if (res.code === 200) profile.value = res.data; const statusRes = await getUserOnlineStatus(route.params.userId); if (statusRes.code === 200) onlineStatus.value = statusRes.data?.status || 0 } finally { loading.value = false } }
async function handleAddFriend() { const res = await addFriend({ friendId: route.params.userId, applyMsg: applyMsg.value }); if (res.code === 200) { showApplyDialog.value = false; showToast('申请已发送') } else showToast(res.message || '操作失败', 'error') }
async function handleBlacklist() { const res = await addBlacklist(route.params.userId); if (res.code === 200) showToast('已拉黑该用户'); else showToast(res.message || '操作失败', 'error') }
function getProfileInitial() { return profile.value?.userNickname?.charAt(0) || '用' }
function getUserIntro() { return profile.value?.userIntro || '这个人很安静，什么都没留下。' }
function formatCreateTime(time) { return time ? new Date(time).toLocaleString('zh-CN') : '未知' }
function openPrivateChat() { router.push(`/chat/private/${route.params.userId}`) }
function openProfileEdit() { router.push('/profile') }
onMounted(load)
</script>
...
<template>
  <div class="page user-profile-page">
    ...
    <div v-else-if="profile">
      <el-alert v-if="toast.msg" :title="toast.msg" :type="toast.type" show-icon class="toast"
        @close="toast.msg = ''" />
      <el-card class="profile-card" shadow="hover">
        ...
        <div class="actions" v-if="!isSelf()">
          ...
          <el-button type="danger" @click="handleBlacklist">
            <el-icon>
              <Warning />
            </el-icon>
            <span>拉黑</span>
